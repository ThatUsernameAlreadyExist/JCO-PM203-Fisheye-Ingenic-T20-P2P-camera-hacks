...
if [ -n "$F_script" ]; then
  ...
  if [ -e "$SCRIPT_HOME/$script" ]; then
    case "$F_cmd" in
      start)
        ...
        ;;  
      disable)
        rm "/opt/media/sdc/config/autostart/$script"
      ...
      stop)
        ...
        ;;
      enable)
        echo "#!/bin/sh" > "/opt/media/sdc/config/autostart/$script"
        echo "$SCRIPT_HOME$script" >> "/opt/media/sdc/config/autostart/$script"
        echo "Content-type: application/json"
        echo ""
        echo "{\"status\": \"ok\"}"
        ;;
  ...
  if [ ! -d "$SCRIPT_HOME" ]; then
    ...
    for i in $SCRIPTS; do
      ...
      if [ -x "$SCRIPT_HOME/$i" ]; then
        ...
        if [ -f "/opt/media/sdc/config/autostart/$i" ]; then
  ...
  script=$(cat /opt/media/sdc/www/scripts/scripts.cgi.js)
