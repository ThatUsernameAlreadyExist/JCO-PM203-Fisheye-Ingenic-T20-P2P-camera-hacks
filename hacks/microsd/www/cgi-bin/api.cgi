...
# START VARIABLES
PATH="/system/bin:/bin:/usr/bin:/sbin:/usr/sbin:/media/mmcblk0p2/data/bin:/media/mmcblk0p2/data/sbin:/media/mmcblk0p2/data/usr/bin:/lib/:/ipc/lib/:/opt/media/sdc/lib/"
CONFIGPATH=/opt/media/sdc/config
BINPATH=/opt/media/sdc/bin
SDPATH=/opt/media/sdc
VERSION="v0.0.5 Beta"
# END VARIABLES
...
# Control Audio
  audio_test)
    $BINPATH/audioplay /opt/media/sdc/media/police.wav &
...
# Control Video
  ...
  get_snaphot)
    if [ $F_raw == 1 ]; then
      $BINPATH/getimage > /var/run/snaphot.jpg
    else
      if [ `ps | grep v4l2rtspserver-master | grep -v grep | wc -l` -eq 1 ]; then
        ...
        PARAMS="$PARAMS -f image2 /var/run/snaphot.jpg"
  ...
  \"snaphot\": \"`cat /var/run/snaphot.jpg | $BINPATH/busybox base64 | tr -d '\n'`\"
}"
        rm /run/snaphot.jpg
      else
        echo "Content-type: image/jpg; charset=utf-8; Pragma: no-cache; Expires: Wednesday, 27-Dec-95 05:29:10 GMT"
        echo ""
        cat /var/run/snaphot.jpg
        rm /var/run/snaphot.jpg
      fi
    else
